Hi, fellow humans! This is Hema :D

I'm a Full Stack Developer with Salesforce, AWS & QA Automation background.

I enjoy building web and mobile applications, as well as exploring new technologies and coding. I also take pride in teaching - it has helped me see how every human being is capable and I apply my empathetic approach in every teaching moment possible - be it personal or professional.

Values guide the human race to build a sustainable world and it is important for me that the company I work for has great values not just in writing, but in its applications as well.

That's a little about me. Thanks for reading!

Cheers!

---

🛠️ Tech Stack
---


| Category                     | Technologies & Tools |
|-----------------------------|-----------------------|
| **Front-End**               | HTML5, CSS3, JavaScript, React, Flutter, Figma |
| **Back-End**                | Python, MySQL |
| **Version Control & Collaboration** | Git, GitHub, Jira |
| **Cloud & Deployment**      | AWS Certified Cloud Practitioner, BrowserStack |
| **Testing & Automation**    | Jest, Cypress, Selenium, BrowserStack |
| **Testing Approaches**      | Unit, TDD, BDD, End-to-End (E2E), Page Object Model (POM) |
| **Certifications in Progress** | Salesforce JavaScript Developer I, AWS Certified Cloud Practitioner |
| **Methodologies**           | Agile, Scrum, Kanban |

📫 Contact Me
---

**LinkedIn:** [Hema P.](https://es.linkedin.com/in/hemaps)  
**Email:** [hemapriyaweb@gmail.com](mailto:hemapriyaweb@gmail.com)
